use std::{arch::asm, fmt::Write, num::NonZeroUsize, sync::Mutex};
...
pub static BREAKPOINTS: Mutex<BreakpointManager> = Mutex::new(BreakpointManager::new());
...
impl BreakpointManager {
    ...
    pub unsafe fn register(&mut self, addr: usize, thumb: bool) -> Result<(), BreakpointError> {
    ...
    /// this exception will continue execution.
    ///
    ...
    pub unsafe fn prepare_for_continue(&mut self, idx: usize) {
...
pub(crate) unsafe fn handle_breakpoint(fault: &mut Fault<'_>) {
    ...
    if let Ok(mut breakpoints) = BREAKPOINTS.try_lock()
        && let Some(idx) = breakpoints.query_address(fault.ctx.program_counter)
    {
        // This `bkpt` instruction is a placeholder tracked by our breakpoint manager. Let's replace
        // it and continue execution. In the future we may want to pause and enter a terminal or
        // something.

        is_explicit_bkpt = false;
        unsafe {
            breakpoints.prepare_for_continue(idx);
        }
    }

    if is_explicit_bkpt {
        // If the breakpoint was caused by an explicit `bkpt` instruction, then we don't actually
        // want to directly return to it since it'd just cause another breakpoint as soon as we do.
        // So, we need to advance by one instruction to skip past it.
