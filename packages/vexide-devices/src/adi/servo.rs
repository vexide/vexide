...
//! # Hardware Overview
//!
//! Servos are similar in both appearance and function to [`AdiMotor`](super::motor::AdiMotor)s,
...
impl AdiServo {
    ...
    /// # Range
    ///
    /// VEX servos have an operating range of 100° spanning from [`AdiServo::MIN_POSITION`] (-50°)
    /// to [`AdiServo::MAX_POSITION`] (50°). Values outside of this range will be saturated at
    /// their respective min or max value.
    ///
    /// # Errors
    ///
    ...
    /// Sets the servo's raw position using a raw 8-bit PWM input from [-127, 127]. This is
    /// functionally equivalent to [`Self::set_target`] with the exception that it accepts an
    /// unscaled integer rather than an [`Angle`].
    ///
    /// # Range
    ///
    /// VEX servos have an operating range of 100° spanning from [`AdiServo::MIN_POSITION`] (-127)
    /// to [`AdiServo::MAX_POSITION`] (127).
    ///
    /// # Errors
    ///
    /// These errors are only returned if the device is plugged into an
