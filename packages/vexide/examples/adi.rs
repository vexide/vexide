...
async fn main(peripherals: Peripherals) {
    ...
    let potentiometer = AdiPotentiometer::new(peripherals.adi_a, PotentiometerType::V2).unwrap();

    // Create a line tracker on triport B.
    let line_tracker = AdiLineTracker::new(peripherals.adi_b).unwrap();

    // Create an ultrasonic range finder on triport C.
    let ultrasonic = AdiUltrasonic::new((peripherals.adi_c, peripherals.adi_d)).unwrap();
